-- Checking if the season data is complete - each team should have played twice against their opponents (a home match and an away match)
select 
	Season_End_Year, 
...
select 
    ...
    cast((count(CASE WHEN FTR = "H" THEN FTR END)/count(Season_End_Year)*100) as decimal (10,2)) as Home_Wins_Percentage,
    count(CASE WHEN FTR = "D" THEN FTR END) as Draws, 
    cast((count(CASE WHEN FTR = "D" THEN FTR END)/count(Season_End_Year)*100) as decimal (10,2)) as Draws_Percentage,
    count(CASE WHEN FTR = "A" THEN FTR END) as Away_Wins, 
    cast((count(CASE WHEN FTR = "A" THEN FTR END)/count(Season_End_Year)*100) as decimal (10,2)) as Away_Wins_Percentage
from eplmatches
group by Season_End_Year
